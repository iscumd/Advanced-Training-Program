...
{
    kI2CFileDescriptor = wiringPiI2CSetup(kI2CAddress);
    if (kI2CFileDescriptor < 0)
	return false;
   else if (kI2CFileDescriptor >= 0)
      return true ;
}
...
{
    int toReturn = wiringPiI2CReadReg8(kI2CFileDescriptor, readRegister);
    if (toReturn < 0) {
...
{   // For debugging:
    ...
    int toReturn = wiringPiI2CWriteReg8(kI2CFileDescriptor, writeRegister, writeValue);
    if (toReturn < 0) {
